...
`);
        ...
        if (!assignCount || assignCount.type !== "assignGather" || !assignId || assignId.type !== "assignGather")
            return;
...
`,
            {
                civDslByName: {
                    DemoCiv: ["grant food 25", "modifier action.cost.build_house_plain.wood mul 0.5"],
                },
            },
        );
        expect(build.commands).toHaveLength(2);
...
`);
        ...
        const gathersForestAtStart = villager2?.segments.some(
            (s) => s.kind === "gather" && s.detail === "wood:forest" && s.start === 0,
        );
...
after completed lure_boar assign to boar
after depleted boar assign to sheep
...
`);
        ...
        const lurerOnBoar = v1?.segments.some((s) => s.kind === "gather" && s.detail === "food:boar");
        const hadBoarPhase = v2?.segments.some((s) => s.kind === "gather" && s.detail === "food:boar");
        const returnedToSheep = v2?.segments.some(
            (s) => s.kind === "gather" && s.detail === "food:sheep" && s.start >= 2,
        );
...
`);
        ...
        const villager1 = result.entityTimelines["villager-1"];
        const villager2 = result.entityTimelines["villager-2"];
        const v1LureAt0 = villager1?.segments.some(
            (s) => s.kind === "action" && s.detail === "lure_boar" && s.start === 0,
        );
        const v2LureAt0 = villager2?.segments.some(
            (s) => s.kind === "action" && s.detail === "lure_boar" && s.start === 0,
        );
...
`);
        ...
        const villager2 = result.entityTimelines["villager-2"];
        const v1LureActions =
            villager1?.segments.filter((s) => s.kind === "action" && s.detail === "lure_boar").length ?? 0;
        const v2LureActions =
...
`);
        ...
        const villager1 = result.entityTimelines["villager-1"];
        const villager2 = result.entityTimelines["villager-2"];
        const v1Forest = villager1?.segments.some(
            (s) => s.kind === "gather" && s.detail === "wood:forest" && s.start >= 34,
        );
        const v2Forest = villager2?.segments.some(
            (s) => s.kind === "gather" && s.detail === "wood:forest" && s.start >= 34,
        );
...
`);
        ...
        const villager2 = result.entityTimelines["villager-2"];
        const autoHouseActions =
            villager2?.segments.filter((s) => s.kind === "action" && s.detail === "build_house_plain") ?? [];
