...
export function processTriggers(
  ...
  options: SimOptions,
  event: TriggerEvent,
  executeCommand: (state: SimState, game: GameData, options: SimOptions, cmd: BuildOrderCommand, commandIndex: number, triggerContext?: TriggerEventContext) => void,
): void {
  for (const rule of state.triggerRules) {
    if (rule.trigger.kind === "completed") {
      ...
      const allDepleted = matchingNodes.every((node) => node.depleted || ((node.remainingStock ?? Infinity) <= EPS));
...
export function processBoundaryPhases(
  ...
  callbacks: {
    onComplete: (state: SimState, game: GameData, actionId: string, actors: string[]) => { createdNodeIds: string[] };
    onDeferred: () => void;
    onAutomation: () => void;
  ...
  for (const completion of completions) {
    queue.push(state.now, "completion", { kind: "completion", actionId: completion.actionId, actors: [...completion.actors] });
  }
