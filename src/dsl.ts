...
const TRIGGER_KEYWORDS = new Set(["clicked", "completed", "depleted", "exhausted"]);
...
function parseCommandPrefix(tokens: string[], lineNo: number): { at: number; rest: string[] } {
    if (tokens[0] === "at") {
        ...
        return { at: parseTimeValue(tokens[1] ?? "", lineNo), rest: tokens.slice(2) };
    }
    if (tokens[0] === "after") {
...
function normalizeActorSelector(entry: string, lineNo: number): string {
    ...
    if (/\s/.test(entry)) {
        throw new Error(
            `Line ${lineNo}: invalid actor selector '${entry}'. Use '<type>', '<type> <n>', or '<type>-<n>'.`,
        );
    }
    return entry;
}
...
function parseTriggerCondition(
    ...
    throw new Error(
        `Line ${lineNo}: unknown trigger '${kind}'. Use 'clicked', 'completed', 'depleted', or 'exhausted'.`,
    );
}
...
export function parseBuildOrderDsl(input: string, options?: ParseBuildOrderDslOptions): BuildOrderInput {
    ...
    let scores: ScoreCriterion[] | undefined;
    ...
    for (let idx = 0; idx < lines.length; idx += 1) {
        ...
        if (tokens[0] === "evaluation") {
            ...
            evaluationTime = parseTimeValue(tokens[1], lineNo);
            continue;
        }
        ...
        if (tokens[0] === "start" && tokens[1] === "with") {
            ...
            for (const entry of entries) {
                ...
                if (!m) {
                    throw new Error(
                        `Line ${lineNo}: invalid start-with item '${entry}'. Use '<entityType>' or '<entityType> <count>'.`,
                    );
                }
        ...
        if (tokens[0] === "score") {
            ...
            if (method !== "time")
                throw new Error(`Line ${lineNo}: unknown score method '${method}'. Only 'time' is supported.`);
            ...
            if (maybeCount) {
                if (maybeCount === "x" && tokens[5]) {
                    count = parseNumber(tokens[5], lineNo);
                } else if (maybeCount.startsWith("x")) {
                    count = parseNumber(maybeCount.slice(1), lineNo);
                } else {
                    throw new Error(`Line ${lineNo}: unexpected token '${maybeCount}' after score target.`);
                }
            }
            const condition = parseTriggerCondition(condKind, condTarget, lineNo, selectorAliases);
            if (!scores) scores = [];
            scores.push({ method: "time", condition, ...(count !== undefined ? { count } : {}) });
            continue;
        }
            ...
            for (let i = 2; i < rest.length; i += 1) {
                ...
                if (t === "from") {
                    ...
                    if (selectors.length === 0)
                        throw new Error(`Line ${lineNo}: queue 'from' requires at least one selector.`);
        ...
        if (op === "assign") {
            ...
            if (rest[1] === "event" || (trigger && rest[1] === "to")) {
                const selectors = rest
                    .slice(toIdx + 1)
                    .map((raw) =>
                        raw === "created" ? "id:created" : (parseDslSelectors([raw], selectorAliases)[0] ?? ""),
                    );
                if (selectors.some((x) => !x))
                    throw new Error(`Line ${lineNo}: invalid selector in 'assign event ...'.`);
            ...
            const fromSelectors =
                fromIdx >= 0 ? parseDslSelectors(rest.slice(fromIdx + 1, toIdx), selectorAliases) : undefined;
        ...
        if (op === "auto-queue") {
            ...
            for (let i = 2; i < rest.length; i += 1) {
                ...
                if (t === "from") {
                    const selectors = parseDslSelectors(rest.slice(i + 1), selectorAliases);
                    if (selectors.length === 0)
                        throw new Error(`Line ${lineNo}: auto-queue 'from' requires at least one selector.`);
                    actorResourceNodeSelectors = selectors;
                    i = rest.length;
                    continue;
                }
        ...
        if (op === "stop-auto-queue") {
            ...
            for (let i = 2; i < rest.length; i += 1) {
                ...
                if (t === "using") {
                    ...
                    if (selector.match(/^(.*)-(\d+)$/)) {
                        throw new Error(
                            `Line ${lineNo}: stop-auto-queue 'using' must be an actor type, not a specific ID.`,
                        );
                    }
                    actorType = selector;
                    i += parsed.consumed;
                ...
                if (t === "from") {
                    const selectors = parseDslSelectors(rest.slice(i + 1), selectorAliases);
                    if (selectors.length === 0)
                        throw new Error(`Line ${lineNo}: stop-auto-queue 'from' requires at least one selector.`);
                    actorResourceNodeSelectors = selectors;
                    i = rest.length;
                    continue;
                }
    ...
    if (scores !== undefined) out.scores = scores;
    return out;
}
