...
import { AutoQueueRule, EPS, QueueRule, SimState, compareEntityIdNatural, quantizeDuration, switchEntityActivity, toFutureTick } from "./sim_shared";
...
export function tryScheduleActionNow(
  ...
  game: GameData,
  options: SimOptions,
  cmd: Pick<Extract<BuildOrderCommand, { type: "queueAction" }>, "actionId" | "actorSelectors" | "actorResourceNodeIds" | "actorResourceNodeSelectors">,
): { status: "scheduled"; completionTime: number } | { status: "blocked"; reason: "NO_ACTORS" | "INSUFFICIENT_RESOURCES" | "POP_CAP" } | { status: "invalid"; message: string } {
  ...
  const actorCount =
    (cmd.actorSelectors && cmd.actorSelectors.length > 0)
      ? cmd.actorSelectors.length
      : (action.actorCount ?? 1);
  ...
  if (cmd.actorResourceNodeSelectors !== undefined) actorRequest.actorResourceNodeSelectors = cmd.actorResourceNodeSelectors;
  ...
  const singleWorkerDuration = applyNumericModifiers(action.duration, [`action.duration.${action.id}`], state.activeModifiers);
  ...
  return { status: "scheduled", completionTime: state.now + duration };
}
...
function commandScheduledResult(
  rule: QueueRule,
  startedAt: number,
): CommandResult {
  return {
...
function computeBlockedNextAttempt(
  ...
  game: GameData,
  options: SimOptions,
  rule: Pick<QueueRule | AutoQueueRule, "actionId" | "actorSelectors" | "actorResourceNodeIds" | "actorResourceNodeSelectors">,
  ...
  const resourceFloorOverrides = populationResource
    ? { [populationResource]: game.population?.floor ?? 0 }
    : undefined;
  const dtToAfford = timeToAffordWithCurrentRates(state.resources, action.costs ?? {}, econ.resourceRates, options.debtFloor, resourceFloorOverrides);
...
export function registerQueueAction(
  ...
  if (cmd.actorResourceNodeSelectors !== undefined) rule.actorResourceNodeSelectors = [...cmd.actorResourceNodeSelectors];
...
export function processQueueRules(
  ...
  do {
    ...
    for (const rule of [...state.queueRules]) {
      ...
      const queueCmd: Pick<Extract<BuildOrderCommand, { type: "queueAction" }>, "actionId" | "actorSelectors" | "actorResourceNodeIds" | "actorResourceNodeSelectors"> = {
      ...
      if (rule.actorResourceNodeSelectors !== undefined) queueCmd.actorResourceNodeSelectors = rule.actorResourceNodeSelectors;
      ...
      if (options.strict && (result.reason === "INSUFFICIENT_RESOURCES" || result.reason === "POP_CAP")) {
        ...
        state.commandResults.push(commandFailureResult(rule, `Could not schedule iteration ${iter}/${rule.totalIterations}.`));
        state.queueRules = state.queueRules.filter((r) => r !== rule);
        continue;
      }
...
export function registerAutoQueue(
  ...
  const existingIdx = state.autoQueueRules.findIndex(
    (r) =>
      ...
      JSON.stringify(r.actorResourceNodeSelectors ?? []) === JSON.stringify(rule.actorResourceNodeSelectors ?? []),
  );
  if (existingIdx >= 0) {
...
export function stopAutoQueue(
  ...
  state.autoQueueRules = state.autoQueueRules.filter((rule) => {
    ...
    if (cmd.actorType !== undefined && JSON.stringify(rule.actorSelectors ?? []) !== JSON.stringify([cmd.actorType])) return true;
    if (cmd.actorResourceNodeIds !== undefined && JSON.stringify(rule.actorResourceNodeIds ?? []) !== JSON.stringify(cmd.actorResourceNodeIds)) return true;
    if (cmd.actorResourceNodeSelectors !== undefined && JSON.stringify(rule.actorResourceNodeSelectors ?? []) !== JSON.stringify(cmd.actorResourceNodeSelectors)) return true;
    return false;
  });
  pushCommandScheduledResult(state, commandIndex, cmd.type, requestedAt);
}
...
export function processAutoQueue(
  ...
  do {
    ...
    for (const rule of [...state.autoQueueRules]) {
      ...
      if (blockedActorTypes && blockedActorTypes.length > 0 && ruleAction.actorTypes.some((t) => blockedActorTypes.includes(t))) {
        if (shouldDebugAction(rule.actionId)) {
          simDebug("processAutoQueue.skip", `action=${rule.actionId}`, "reason=BLOCKED_ACTOR_TYPE", `now=${state.now}`);
        }
        continue;
      }
      ...
      if (result.status === "scheduled") {
        ...
        changed = true;
        if (shouldDebugAction(rule.actionId)) {
          simDebug("processAutoQueue.scheduled", `action=${rule.actionId}`, `now=${state.now}`, `nextAttemptAt=${rule.nextAttemptAt}`);
        }
        continue;
      }
...
export function assignGather(
  ...
  if (cmd.actorResourceNodeSelectors !== undefined) assignRequest.actorResourceNodeSelectors = cmd.actorResourceNodeSelectors;
  ...
  const assignResult = assignGatherByEntityIds(
    state,
    actorIds,
    cmd.resourceNodeIds,
    cmd.resourceNodeSelectors,
  );
  if (!assignResult.ok) {
