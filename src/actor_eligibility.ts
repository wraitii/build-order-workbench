...
function resolveAllowedActorNodeIds(
  ...
  actorResourceNodeIds?: string[],
  actorResourceNodeSelectors?: string[],
): { hasFilter: boolean; allowedNodeIds?: Set<string>; allowIdle: boolean } {
  let allowIdle = false;
  const hasFilter =
  ...
  if (
    ...
    return { hasFilter: false, allowIdle };
  }
...
function isEligibleEntity(
  ...
  nodeFilter: { hasFilter: boolean; allowedNodeIds?: Set<string>; allowIdle: boolean },
): boolean {
...
function selectBySelectors(
  ...
  for (const selector of request.actorSelectors ?? []) {
    ...
    const candidates = state.entities
      .filter((e) => !used.has(e.id) && e.entityType === selector && isEligibleEntity(state, e, request, nodeFilter))
      .sort((a, b) => {
...
export function pickEligibleActorIds(
  state: SimState,
  request: ActorEligibilityRequest,
): string[] {
...
export function nextEligibleActorAvailabilityTime(
  ...
  const actorCount = actorSelectors && actorSelectors.length > 0
    ? actorSelectors.length
    : request.actorCount;
