import { addResources, cloneResources, countEntitiesByType, findNextEventTime, normalizeCommandTimes, recordEntityCountPoint, SimState, switchEntityActivity, EPS, toFutureTick, toTick, TIME_STEP_SECONDS } from "./sim_shared";
import { BuildOrderInput, BuildOrderCommand, GameData, SimOptions, SimulationResult, TriggerExecutableCommand } from "./types";
import { NodeDepletionEvent, applyStockModifierToExistingNodes, advanceTime, computeEconomySnapshot, instantiateResourceNode } from "./economy";
import { assignEntityToGatherTargets, assignGather, assignGatherByEntityIds, finalizeQueueRulesAtEvaluation, processAutoQueue, processQueueRules, registerAutoQueue, registerQueueAction, setSpawnGatherRule, stopAutoQueue } from "./scheduler";
import { EventQueue } from "./event_queue";
import { TriggerEventContext, processBoundaryPhases } from "./sim_phases";
...
function onEventComplete(state: SimState, game: GameData, actionId: string, actors: string[]): { createdNodeIds: string[] } {
...
function processAutomation(state: SimState, game: GameData, options: SimOptions): void {
  processQueueRules(state, game, options);
...
function advanceWithAutomation(
  ...
  while (state.now + EPS < targetTime) {
    ...
    const nextDepletion = econ.nextDepletionTime !== undefined ? toFutureTick(econ.nextDepletionTime) : Number.POSITIVE_INFINITY;
...
function resolveAssignRequestedCount(state: SimState, cmd: Extract<BuildOrderCommand, { type: "assignGather" }>): number {
  if (cmd.all) {
    ...
    if (cmd.actorResourceNodeSelectors !== undefined) allRequest.actorResourceNodeSelectors = cmd.actorResourceNodeSelectors;
...
function resolveAssignGatherDeferredTime(
  ...
  if (cmd.actorResourceNodeSelectors !== undefined) availability.actorResourceNodeSelectors = cmd.actorResourceNodeSelectors;
...
function executeCommand(
  ...
  const handlers: Record<BuildOrderCommand["type"], () => void> = {
    queueAction: () => {
    let queueCmd: Extract<BuildOrderCommand, { type: "queueAction" }> = cmd as Extract<BuildOrderCommand, { type: "queueAction" }>;
    ...
    assignEventGather: () => {
      ...
      if (actorIds.length === 0) {
        pushInvalidAssignment(assignEventCmd.type, requestedAt, "assign event requires an active trigger event with actors.");
        return;
      }
      ...
      const selectorTargets = (assignEventCmd.resourceNodeSelectors ?? []).filter((selector) => selector !== "id:created");
...
export function runSimulation(game: GameData, buildOrder: BuildOrderInput, options: SimOptions): SimulationResult {
  ...
  const popResource = game.population?.resource;
  if (popResource && !(buildOrder.startingResources && Object.prototype.hasOwnProperty.call(buildOrder.startingResources, popResource))) {
